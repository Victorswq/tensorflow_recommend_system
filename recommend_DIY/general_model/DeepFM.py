...
class DeepFM(AbstractRecommender):
    ...
    def build_net(self):
        ...
        self.feature_matrix = tf.get_variable(name="feature_matrix", shape=[self.feature_max_len, self.embedding_size],
                                              initializer=init_w)
        self.feature_matrix_first_order = tf.get_variable(name="feature_matrix_first_order",
                                                          shape=[self.feature_max_len, 1], initializer=init_w)

        # embedding and variables
        self.feature_embedding = tf.nn.embedding_lookup(self.feature_matrix,
                                                        self.feature_index)  # N * feature_size * embedding_size
        self.feature_value = self.feature_value[tf.newaxis, -1]  # N * feature_size * 1
        self.feature_embedding = tf.multiply(self.feature_embedding,
                                             self.feature_value)  # N * feature_size *embedding_size

        self.feature_embedding_first_order = tf.nn.embedding_lookup(self.feature_embedding_first_order,
                                                                    self.feature_index)  # N * feature_size * 1
        self.feature_embedding_first_order = tf.squeeze(self.feature_embedding_first_order)
        self.feature_embedding_first_order = tf.multiply(self.feature_embedding_first_order,
                                                         self.feature_value)  # N * feature_size
        ...
        self.FM_second = 0.5 * (
            tf.subtract(self.sum_square, self.square_sum, name="sum_square_subtract_square_sum"))  # N * embedding_size
        self.FM_second = tf.layers.dropout(self.FM_second, rate=self.dropout_layers[0],
                                           name='dropout_layer_input_FM_for_weight')

        # AFM_net
        self.layers[-1] = self.feature_size
        self.feature_weight = self.FM_second
        for i in range(len(self.layers)):
            self.feature_weight = tf.layers.dense(self.feature_weight, self.layers[i], activation=tf.nn.relu,
                                                  name="attention_layer_%d" % i)  # N * feature_size
            self.feature_weight = tf.layers.dropout(self.feature_weight, rate=self.dropout_layers[i],
                                                    name="dropout_layer_%d" % i)
        self.feature_weight = self.feature_weight[tf.newaxis, -1]
        self.FM_with_weight = tf.multiply(self.feature_weight,
                                          self.feature_embedding)  # N * feature_size * embedding_size
        ...
        # prediction
        self.inference = tf.layers.dense(self.embedding_for_prediction, 1, activation=tf.nn.sigmoid,
                                         name="prediction")  # N * 1
